...
class HardwareCapabilities:
    ...
    triton_streaming_dh64_attention: bool | None = None
...
class PlanBuilder:
    ...
    def _validate_runtime(
        ...
        if runtime is RuntimeBackend.STREAMED:
            from .shape14.planning import validate_streamed_runtime

            return validate_streamed_runtime(config, context, reject)
    ...
    def _validate_attention(
        ...
        *,
        ...
        if backend is AttentionBackend.TRITON_STREAMING_DH64:
            from .shape14.planning import validate_streaming_attention

            validate_streaming_attention(
                config=config,
                context=context,
                capability=hardware.triton_streaming_dh64_attention,
                qkv_dtype=qkv_dtype,
                reject=reject,
            )
            return
    ...
    def _validate_ffn(
        ...
        if backend is FFNBackend.TRITON_FUSED_MLP_BOUNDARY:
            ...
            if (
                context.batch_size * context.seq_len != 2048
                ...
                reject(
                    "unsupported_shape",
                    "program.ffn",
                    "fused MLP boundary requires 2048 rows with D=F=128",
                )
