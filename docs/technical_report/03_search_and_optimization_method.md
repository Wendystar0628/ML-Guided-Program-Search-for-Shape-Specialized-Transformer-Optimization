...
Only Screen measurements train TPE. The minimized objective is measured median latency, while three continuous constraints independently encode:
...
The nominal per-Shape wall-time budget is divided into 65% Screen, 17% Enhanced, and 18% Formal time. Every mandatory structure first receives a Screen witness. Branches are ranked by their best feasible measured Screen median; the scheduler does not invent future learning-curve gains.
...
Under the documented null assumption that a block has at most 0.5 probability of meeting the 2% target, the union of the three one-sided false-promotion bounds remains below 0.05. A challenger is rejected early when it can no longer reach the final 11-win condition. Both programs must remain feasible. When no incumbent exists, one feasible Formal result may establish the first deployment.
