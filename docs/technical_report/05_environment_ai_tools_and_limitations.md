...
3. **Incomplete interaction ablation due to time limits.** The report evaluates every resident Shape with legal one-family-at-a-time knockouts, but the remaining competition time did not permit a full combinatorial ablation over mechanism subsets. Figure 8 therefore measures removal sensitivity; it does not identify additive contribution shares or fully resolve higher-order interactions among runtime, layout, precision, fusion, and Kernel choices.
4. **Shape 14 official oracle is unavailable.** The streamed implementation passes a local `B=1` semantic-equivalence check and completes the full logical `B=32` workload, but the promised official `B=32` input/output artifact and protocol are not present in the supplied materials. The sampled execution digest is a reproducibility marker, not a substitute for the pending official elementwise comparison.
5. **Single-platform empirical evidence.** All reported performance evidence comes from one native-Windows RTX 4080 system. The architecture retains hardware discovery and exact-device deployment boundaries, but performance portability across NVIDIA generations, operating systems, data-centre GPUs, or non-CUDA accelerators has not been demonstrated.
6. **No learned cross-device or cross-workload performance model.** Compatible search history and deterministic Shape similarity provide warm starts, but the project has not learned a transferable cost model from multiple devices and workload families. A materially different target still requires substantial fresh measurement.
7. **Offline optimization cost.** Candidate compilation, correctness evaluation, and isolated GPU benchmarking consume dedicated device time. Persistent studies amortize repeated work, but the present method is designed for offline deployment selection rather than millisecond-scale online adaptation to rapidly changing workloads.
8. **Closed synthesis vocabulary and production boundary.** Program synthesis recombines the PyTorch operators, Triton kernels, fusions, layouts, and runtimes already exposed by the repository. It cannot discover a fundamentally new algorithm or Kernel family until that primitive is implemented, and the deployed registry has not yet been integrated with a multi-tenant serving system where concurrency, tail latency, power, and operational failure modes may change the optimum.
...
- validate Shape 14 chunk by chunk against the official full-batch input/output artifact when its format becomes available;
