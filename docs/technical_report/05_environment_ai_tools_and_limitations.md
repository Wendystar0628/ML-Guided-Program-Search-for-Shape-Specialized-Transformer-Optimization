...
4. **Official timing protocol remains incomplete.** The project-defined median, P90, warmup, repeat counts, GPU isolation, and timestamped outputs are explicit in the [final-performance test contract](../04_最终交付物/01_最终性能测试/README.md) and the repository [reproduction procedure](../../README.md#reproducing-the-declared-result). The remaining uncertainty is the unpublished final official timing and aggregation protocol.
5. **Single declared input variant.** The benchmark and runtime accept parameterized padding and input scale, but the declared performance snapshot covers only FP32 input/output with zero padding and unit input scale. Future official mask or input updates require fresh validation and, where the deployment identity changes, fresh search.
