...
每个 Shape 仍使用新的 Python 进程，两个任务与搜索、Profile 和其他 Benchmark 共享同一 GPU 排他锁。默认 `final` 预设对 Shapes 01–05、07–13 保持正式测量强度；Shape 06 使用 1 次正确性、2 次 Warmup、3 轮每轮 5 次正式计时。Shape 14 的 `smoke` 延迟明确标为低成本 Model-compute Estimate；`formal/final` 只运行一次完整逻辑 Batch，并让每个流式 Microbatch 使用不同输入。Shape 14 的本地 `B=1` 语义检查、完整逻辑 `B=32` 执行状态和官方 `B=32` I/O 验证状态分别记录，不能合并解释成一个笼统的 PASS。
...
Shapes 01–13 展示 Baseline Median、部署方案 Median/P90、Speedup、Peak VRAM 和几何平均加速比。Shape 14 单独展示 Latency 类型、Latency、Peak VRAM、本地 `B=1` 语义检查、完整逻辑 `B=32` 执行状态和官方 I/O 验证状态，不计入几何平均。其 sampled execution digest 只是复现标记，不是正确性证明。搜索和迭代观测位于本地 `observations/`，不上传到 GitHub。
