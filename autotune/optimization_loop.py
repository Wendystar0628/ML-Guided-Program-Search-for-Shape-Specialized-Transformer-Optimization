...
class OptimizationLoopPolicy:
    """Stop after a search-progress plateau or an absolute iteration limit."""

    no_progress_patience: int
    max_iterations: int

    def __post_init__(self) -> None:
        for name in ("no_progress_patience", "max_iterations"):
...
class OptimizationLoop:
    """Repeat full search sweeps while search progress remains productive."""
    ...
    def run(
        ...
        *,
        ...
        for iteration_index in range(1, policy.max_iterations + 1):
            ...
            if no_progress_streak >= policy.no_progress_patience:
                return OptimizationResult(
                    iterations_run=iteration_index,
                    total_deployment_updates=total_deployment_updates,
                    no_progress_streak=no_progress_streak,
                    stop_reason="no_progress_patience",
                    exit_code=0,
                )
